...
def extract_lesion_black_bg(original_img: Image.Image, binary_mask: np.ndarray) -> str:
    ...
    # Natijani PIL rasmga aylantiramiz
    result_img = Image.fromarray(result_np)

    # Rasmni 300x300 ga o'zgartiramiz
    result_img = result_img.resize((300, 300), resample=Image.Resampling.LANCZOS)

    # Byte oqimga saqlaymiz
    buffered = io.BytesIO()
...
async def predict_and_segment(image: UploadFile = File(...)):
    try:
        ...
        # Modeldan chiqish shaklini to‘g‘rilash
        if pred_mask.ndim == 4:
        ...
        # Toza binary maska qilish
        cleaned_mask = clean_mask(pred_mask)

        # Maskani asl rasm o'lchamiga qayta o'lchash
        ...
        # Maskani base64 ga aylantirish
        mask_base64 = mask_to_base64(np.array(mask_img))

        # Lesionni oq fon bilan ajratib olish
